...
class Calibration(object):
    ...
    def corners3d_to_img_boxes_torch(self, corners3d):
        """
        :param corners3d: (N, 8, 3) corners in rect coordinate
        :return: boxes: (None, 4) [x1, y1, x2, y2] in rgb coordinate
        :return: boxes_corner: (None, 8) [xi, yi] in rgb coordinate
        """
        sample_num = corners3d.shape[0]
        corners3d_hom = torch.cat((corners3d, torch.ones((sample_num, 8, 1), device=corners3d.device)), axis = 2)  # (N, 8, 4)

        img_pts = torch.matmul(corners3d_hom, torch.tensor(self.P2.T, device=corners3d.device))  # (N, 8, 3)

        x, y = img_pts[:, :, 0] / img_pts[:, :, 2], img_pts[:, :, 1] / img_pts[:, :, 2]
        
        x1, y1 = torch.min(x, axis = 1)[0], torch.min(y, axis = 1)[0]
        x2, y2 = torch.max(x, axis = 1)[0], torch.max(y, axis = 1)[0]
        boxes = torch.cat((x1.reshape(-1, 1), y1.reshape(-1, 1), x2.reshape(-1, 1), y2.reshape(-1, 1)), axis = 1)
